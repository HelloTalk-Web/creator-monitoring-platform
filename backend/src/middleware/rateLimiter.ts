...
export const apiKeyRateLimiter = createRateLimiter({
  ...
  maxRequests: 100, // 每个用户每分钟最多100个请求
  keyGenerator: (req) => {
    const userId = (req as any).user?.id
    return userId ? `user:${userId}` : getIP(req)
  }
})
...
export const scrapeRateLimiter = createRateLimiter({
  windowMs: 60 * 1000, // 1分钟
  maxRequests: 10, // 每个用户每分钟最多10个抓取请求
  keyGenerator: (req) => {
    const userId = (req as any).user?.id
    return userId ? `scrape:user:${userId}` : getIP(req)
  }
})
...
export const authRateLimiter = createRateLimiter({
  windowMs: 15 * 60 * 1000, // 15分钟
  maxRequests: 5, // 每个IP每15分钟最多5次登录尝试
